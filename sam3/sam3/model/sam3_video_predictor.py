...
class Sam3VideoPredictor:
    ...
    def __init__(
        ...
        # Resolve device: explicit param > CUDA > MPS > CPU
        if device is None:
    ...
    def _get_session_stats(self):
        ...
        # GPU memory stats only available on CUDA
        if torch.cuda.is_available():
            mem_str = (
                f", GPU memory: "
            ...
            mem_str = f", device: {getattr(self, 'device', 'unknown')}"
        session_stats_str = (
            f"live sessions: [{', '.join(live_session_strs)}]{mem_str}"
        )
    ...
    def _get_torch_and_gpu_properties(self):
        ...
        if torch.cuda.is_available():
            ...
            torch_and_gpu_str = (
                f"torch: {torch.__version__}, device: {getattr(self, 'device', 'unknown')}"
            )
