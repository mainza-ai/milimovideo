...
def smart_inference_mode():
    """Returns torch.no_grad() for MPS, and torch.inference_mode() otherwise."""
    if torch.backends.mps.is_available():
...
def euler_denoising_loop(
    ...
    callback_on_step_end:
        Optional callback function called at the end of each step.
        It accepts (step_idx, num_steps, latents) arguments.
    ### Returns
    tuple[LatentState, LatentState]
    ...
    for step_idx, _ in enumerate(tqdm(sigmas[:-1])):
        ...
        if callback_on_step_end is not None:
             callback_on_step_end(step_idx, len(sigmas[:-1]), video_state, audio_state)
